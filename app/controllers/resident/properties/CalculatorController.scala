...
class CalculatorController @Inject() (
  ...
  def calculateTotalGain(propertyGainModel: PropertyTotalGainModel): Action[AnyContent] = Action.async {
    ...
    val result = calculationService.calculateGainFlat(
      propertyGainModel.totalGainModel.disposalValue,
  ...
  def calculateChargeableGain(propertyChargeableGainModel: PropertyChargeableGainModel): Action[AnyContent] =
    Action.async {
      ...
      val lettingReliefsUsed            = calculationService.determineLettingsReliefsUsed(
        gain,
        prrUsed,
      ...
      val aeaRemaining                  =
        calculationService.annualExemptAmountLeft(propertyChargeableGainModel.annualExemptAmount, aeaUsed)
      val allowableLossesRemaining      = calculationService.determineLossLeft(
        gain - (lettingReliefsUsed + prrUsed),
        round("up", propertyChargeableGainModel.allowableLosses.getOrElse(0))
      )
      ...
      val broughtForwardLossesUsed      = calculationService.calculateAmountUsed(
        round("up", propertyChargeableGainModel.broughtForwardLosses.getOrElse(0)),
      ...
      val deductions = round(
        "up",
        prrUsed + lettingReliefsUsed + round("up", allowableLossesUsed) + aeaUsed + round(
          "up",
  ...
  def calculateTaxOwed(propertyCalculateTaxOwedModel: PropertyCalculateTaxOwedModel): Action[AnyContent] =
    Action.async {
      ...
      val prrUsed                                   = calculationService.determineReliefsUsed(
        gain,
        propertyCalculateTaxOwedModel.propertyChargeableGainModel.prrValue
      )
      val lettingReliefsUsed                        = calculationService.determineLettingsReliefsUsed(
        gain,
        prrUsed,
      ...
      val chargeableGain                            = calculationService.calculateChargeableGain(
        gain,
        lettingReliefsUsed + prrUsed,
        propertyCalculateTaxOwedModel.propertyChargeableGainModel.allowableLosses.getOrElse(0.0),
      ...
      val deductions                                =
        prrUsed + lettingReliefsUsed + propertyCalculateTaxOwedModel.propertyChargeableGainModel.allowableLosses
          .getOrElse(0.0) +
      ...
      val calculationResult: CalculationResultModel = calculationService.calculationResult(
        ...
        calculationService.brRemaining(
          propertyCalculateTaxOwedModel.previousIncome,
          propertyCalculateTaxOwedModel.personalAllowance,
          propertyCalculateTaxOwedModel.previousTaxableGain.getOrElse(0.0),
  ...
  def calculateTotalCosts(propertyTotalGainModel: PropertyTotalGainModel): Action[AnyContent] = Action.async {
    ...
    val result = calculationService.calculateTotalCosts(
      propertyTotalGainModel.totalGainModel.disposalCosts,
